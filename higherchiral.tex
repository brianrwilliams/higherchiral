...
\[
\mathbf{J}^{\mathbb{A}^d}_{{I}}(M_{{I}}):=\Gamma\left(\mathbb{J}_{\mathbb{A}^d}^{{I}},\Omega^{\bullet}_{\mathbb{J}_{\mathbb{A}^d}^{{I}}/\mathrm{Conf}_{{I}}(\mathbb{A}^d)}\otimes p^*_{{I}}j^*_{{I}}M_{{I}} \right)=M_{{I}}\otimes_{\mathcal{O}_{(\mathbb{A}^d)^{{I}}}}\mathbf{J}_{\mathbb{A}^d}^{{I}}.
\]
...
In this subsection, we generalize the concept of residues to the Jouanolou model of $(\omega_{\mathbb{A}^{d}}[d])^{\boxtimes  I}$, where $d\geq1$ is the dimension of the affine space, $ I=\{1,2<\cdots<n\}$ is an ordered set with $n$ elements.
...
We now describe the type of integrals that we are interested in.
\begin{defn}
    Let $(\vec{\Gamma},m,l)$ be a Feynman graph, such that $\Gamma_{0}= I$. Given 
    $$
...
We introduce the following coordinates on $(\mathbb{A}^{d})^{ I}$:
$$
\begin{cases}
...
\begin{proof}
    ...
    Since $\vec{\Gamma}$ is connected, by Proposition \ref{Minverse}, $M_{\vec{\Gamma}}(t)$ is invertible. We notice that the integral over $(\mathbb{A}^d)^{ I-\{n\}}$ is a Gaussian type integral, so
    \begin{align*}
        &W((\vec{\Gamma},m,l),\beta e^{\sum_{i=1}^{n}z_{i}\cdot\lambda_{i}})\\
...
Before we define residues on $\mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$, we state the following useful proposition:
...
\begin{proof}
    ...
    Let $ V= V_{1}+ V_{2}$, where $ V_{1}=\sum_{s=1}^{d}\sum_{i=1}^{n-1}\bar{\tilde{z}}_{i}^{s}\partial_{\bar{\tilde{z}}_{i}^{s}}$, $ V_{2}=\sum_{e\in\vec{\Gamma}_{1}}t_{e}\partial_{t_{e}}$. To simplify notations, we define
    $$
    \tilde{W}=p_{(\vec{\Gamma},m,l)}(-e^{-z_{e}\cdot y_{e}}dy_{e}^{m(e)},-e^{-z_{e}\cdot y_{e}}(z_{e}\cdot dy_{e})dy_{e}^{m(e)})e^{\sum_{i=1}^{n}z_{i}\cdot\lambda_{i}}\otimes \beta.
    $$
    ...
    $$
    We notice that $ V_{2}$ is the infinitesimal generator of $\mathbb{R}^{+}$ action, so $W((\vec{\Gamma},m,l),\beta e^{\sum_{i=1}^{n}z_{i}\cdot\lambda_{i}})$ induces a well-defined differential form on $(0,+\infty)^{\vec{\Gamma}_{1}}/\mathbb{R}^{+}$.
\end{proof}
...
Now, we can define residues on $\mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$.
...
\begin{defn}
    ...
    \end{align*}
    where $(\vec{\Gamma},m,l)$ is a Feynman graph whose corresponding polynomial $p_{(\vec{\Gamma},m,l)}=p(x_{ij}^{s},\mathbf{d}x_{ij}^{s})$. We can define residues for general elements in $\mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$ by linear extension. By Proposition \ref{well-defineness}, residues are well-defined.
\end{defn}

Finally, let's prove a useful property for residues on $\mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$.
\begin{prop}\label{residue 0}
    Let $\alpha\in \mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$, $e^{\sum_{i=1}^{n}z_{i}\cdot\lambda_{i}}\in C^{\infty}((\mathbb{A}^{d})^{ I}\times (\mathbb{A}^{d})^{ I})$, and $1\leq s\leq d$. We have
    ...
    \begin{proof}
        ...
        \end{align*}
        In other words, the formal $\mathcal{D}_{(\mathbb{A}^{d})^{ I}}$-module structure on $\mathbf{J}_{ I}^{\mathbb{A}^{d}}$ coincide with the actual derivative when we compute residues. 
...
\begin{defn}
    Let $\alpha\in \mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})$, the shifted $n$ operation
    $$
    \tilde{\mu}_{ I}:\mathbf{J}_{ I}^{\mathbb{A}^{d}}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I})\rightarrow \omega_{\mathbb{A}^{d}}[d][\mathbb{A}^{d}]\otimes_{k[\lambda_{\bullet}]}k[\lambda_{1},\dots,\lambda_{n}]
    $$
    is given by
    $$
...
\end{defn}
\begin{rem}
    By Proposition \ref{explicit formula}, we know $\tilde{\mu}_{ I}(\alpha)$ is indeed an element of $\omega_{\mathbb{A}^{d}}[d][\mathbb{A}^{d}]\otimes_{k[\lambda_{\bullet}]}k[\lambda_{1},\dots,\lambda_{n}]$, i.e., $\tilde{\mu}_{ I}(\alpha)$ is a polynomial with respect to $\{\lambda_{i}\}_{i\in I}$.
\end{rem}
\begin{prop}
    $\tilde{\mu}_{ I}(\alpha)$ is a homological degree $1$ element in $\mathrm{Hom}^{ch}((\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I},\omega_{\mathbb{A}^{d}}[d])$.
\end{prop}
\begin{proof}
    We first prove $\tilde{\mu}$ is of homological degree $1$. Assume
    $$
    \alpha=p\otimes\beta\in\mathbf{J}_{ I}^{\mathbb{A}^{d}}\otimes_{\mathcal{O}_{(\mathbb{A}^{d})^{ I}}[(\mathbb{A}^{d})^{ I}]}(\omega_{\mathbb{A}^{d}}[d])^{\boxtimes I}[(\mathbb{A}^{d})^{ I}],
    $$
    ...
    Now, let's prove $\tilde{\mu}_{ I}$ is a $\mathcal{D}_{(\mathbb{A}^{d})^{ I}}$-module map. This follows from the following computations:
    \begin{align*}
...
Let's prove $\tilde{\mu}_{ I}$ satisfy the following shifted $L_{\infty}$ relations:
\begin{thm}
...
$$
the shifted $L_{\infty}$ chiral algebra structure $\{\tilde{\mu}_{ I}\}_{ I\in \overrightarrow{\mathbf{fSet}}}$ on $\omega_{\mathbb{A}^{d}}[d]$ corresponds to an $L_{\infty}$ chiral algebra structure on $\omega_{\mathbb{A}^{d}}[d-1]$.
\begin{defn}
    The $L_{\infty}$ algebra structure on $\omega_{\mathbb{A}^{d}}[d-1]$ is called the unit $L_{\infty}$ chiral algebra structure. We denote it by $\mathbf{Unit}[\mathbb{A}^d]$. The corresponding $L_{\infty}$ operations are denoted by $\{\mu^{\mathbf{Unit}}_{ I}\}_{ I\in \overrightarrow{\mathbf{fSet}}}$.
\end{defn}
\subsection{d=2}
