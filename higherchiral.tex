...
$$
The corresponding Jouanolou model $\mathbf{J}_{\mathbb{A}^d}^{({I'}),{I''}}=\Gamma(\mathbb{J}_{\mathbb{A}^d}^{({I'}),{I''}},\Omega^{\bullet}_{\mathbb{J}_{\mathbb{A}^d}^{({I'}),{I''}}/\mathrm{Conf}_{\vec{I}}(\mathbb{A}^d)})$ has the following property
...
\lambda_2$.
\begin{defn}
  We define the chiral operation $\mu_{\{1,2\}}$ for the unit $\omega_{\mathbb{A}^1}$ by the following formula:
    $$
...
\end{align*}
so $\mu_{\{1,2\}}(\alpha)\in \omega_{\mathbb{A}^{1}}\otimes_{k[\lambda_{\bullet}]}k[\lambda_{1},\lambda_{2}].$
...
\begin{prop}
    ...
    $$    \mu_{\{1,2\}}\in \cP^{ch}_{\AA^1}[\omega_{\AA^1}](2) 
    $$
\end{prop}
\begin{proof}
...
\begin{proof}
  ...
  We first verify $\mu_{\{1,2\}}=-\sigma_{12} \cdot \mu_{\{1,2\}}$.   
    Let 
...
Both the Jouanolou model and $\mathcal{A}^{0,\bu}(\mathrm{Conf}_{\vec{I}}(\mathbb{A}^{d}))$ are models for global derived sections of the structure sheaf of $\mathrm{Conf}_{\vec{I}}(\mathbb{A}^{d})$.
The map $i$ is an explicit quasi-isomorphism between these models.
\BW{Can we show this?}
