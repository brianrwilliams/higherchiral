...
Section \ref{s:Jouanolou} begins with an overview of the Jouanolou construction applied to the configuration space of
points in $\mathbb{A}^d$.
...
\begin{equation}\label{}
  \mathrm{Hom}_{\mathcal{D}_{X^{{I}}}}\left(j_{*}j^*\mathcal{A}^{\boxtimes{n}},\Delta_* \mathcal{A}\right)
\end{equation}
where $j \colon \mathrm{Conf}_n(X) \to X \times \cdots \times X$ is inclusion and $\Delta \colon X \to X \times \cdots \times X$ is the diagonal inclusion.
Combining chiral operations of all arities results in an operad.
In genealizing this to $X = \mathbb{A}^d$ we use the Jouanolou trick to provide an explicit model for the $\mathcal{D}
$-module derived pushforward $\mathbf{R} j_* j^* \mathcal{A}$.
From this, we argue that the higher-dimensional derived chiral operations form a dg opeard.
We end the section with a definition of a homotopy chiral algebra in this Jouanolou model.

The next goal of this article is to provide explicit descriptions and examples of chiral algebras in higher dimensions.
Namely, we aim to describe the chiral operations underlying the \textit{unit} chiral algebra on $\mathbb{A}^d$.
To motivate our construction, we recall how the Cauchy residue encodes the chiral operation of the unit chiral algebra
when $d=1$.
In the main part of section \ref{s:feynman}, we generlize the residue to higher-dimensional configuration spaces in the
Jouanolou model.
...
Thus, to a $\cD$-module $\cA$, we obtain the sheaf $h(\cA) = \Omega^\bu_{\mathbb{A}^d} \otimes_{\mathcal{\cD}} \cA$.
We will show that for $\cA$ a chiral algebra in the Jouanolou model the chiral operations endow $h(\mathcal{A})$ with
the structure of a sheaf of $L_\infty$ algebras.

Define the $m$-linear map
$$
...
$$
This induces the $m$-linear map
$$
[-]_m \colon h^{Lie}(\mathcal{F}) \times \cdots \times h^{Lie}(\mathcal{F})\rightarrow h^{Lie}(\mathcal{F}).
$$
