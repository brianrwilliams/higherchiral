...
\Gui{When refer to equation, proposition, we use capitals or not?}
\begin{itemize}
...
\begin{rem}\label{rem:CompareJouanolouPoly}
  ...
  \ref{s:example} depend continuously on the input data. Additionally, the Jouanolou model has a $\mathrm{GL}_d$-symmetry and our later construction is $\mathrm{GL}_d$-equivariant .
  
  \Gui{Comparsion: 1)In polysimplicial model it is relatively easy to prove the existence, but no continuous construction is known.2) Thom-Sullivan does not have the Gln action. 3) the equivalence is not obvious, $\Delta_*\omega$ is not an injective object.}
...
&=F(\lambda_1,\lambda_2,\lambda_3,\lambda_4)\cdot d\mathbf{z}^{\blacklozenge}_o.
  ...
  computation in \cite{Gaiotto:2024gii}.\Gui{Again here } See the Mathematica code attached in the Appendix \ref{MathematicaCode}.
